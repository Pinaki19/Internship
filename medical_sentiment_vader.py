...
def get_medical_sentiment(text):
    ...
    return score, vader_score

def get_final_sentiment(medical_score, vader_score, medical_weight, neg_cutoff, pos_cutoff):
    final_score = (medical_score * medical_weight) + (vader_score * (1 - medical_weight))
...
if __name__ == "__main__":
    ...
    medical_weights = np.linspace(0.1, 0.9, 20)
    neg_cutoffs = np.linspace(-0.5, -0.1, 20)
    pos_cutoffs = np.linspace(0.1, 0.5, 20)
    
    results = []
    
    # Get base scores
    df['medical_score'], df['vader_score'] = zip(*df['note_text'].apply(get_medical_sentiment))
    ...
    # Save results
    results_df.to_csv('vader_medical_sentiment_grid_search.csv', index=False)
